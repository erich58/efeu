...
# r:hdr|
#	:*:header for rules
#	:de:Headerfile für make einbinden
# f:flgs|
#	:*:compiler flags
...
# The normal use is to to prozess a C or mkmf header template with special
...
# Normalerweise wird damit eine Schablone für ein C-Headerfile oder ein
# mkmf-Headerfile mit speziellen Testanweisungen überarbeitet.
...
# or |update|, a template file is evaluatet, otherwise a special
# test is performed.
# :de:
...
# @arglist -i
# 
# :*:The following commands are performed byte the command |$!|
...
if [ $mkmf -ne 0 ]; then
	cat > $tmp/Imakefile << EOF
#include <base.pph>
$rules

tprog: tprog.c
	$CC \$(CFLAGS) $flags -o \$@ tprog.c
EOF
	if	[ $verbose -gt 0 ]; then
		echo "=== rules ===" >&2
		cat $tmp/Imakefile >&2
	...
	(cd $tmp; mkmf; make) 2> $tmp/Error > $tmp/Log
