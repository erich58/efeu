...
{
	Val_ptr(rval) = PG_connect(Val_str(arg[0]));
}
...
static void do_exec (void *rval, void **arg, ExecStatusType stat)
{
	...
	for (list = Val_list(arg[2]); list; list = list->next)
	{
...
static void f_serialize (EfiFunc *func, void *rval, void **arg)
{
	PG_serialize(Val_ptr(arg[0]));
}
...
static EfiFuncDef pg_func[] = {
	...
	{ 0, &Type_void, "PG::serialize ()", f_serialize },
