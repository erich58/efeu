...
int sb_vxprintf (StrBuf *sb, const char *fmt, va_list list)
{
	...
	while (*fmt != 0)
	{
		...
		fmt++;
		key = fmtkey(NULL, fmtkey_pgetc, &fmt);
		...
		case 'S':
			...
			n += fmt_str(sb, key, va_arg(list, char *));
			break;
		...
		case 'C':
			...
			case FMTKEY_LONG:
				...
				break;
			default:
				n += fmt_char(sb, key, va_arg(list, int));
				break;
			}
		...
		case 'n':
			...
			sval = (key->flags & FMT_ALTMODE) ? n : sb->pos - pos;
