/*
	...
	Adapted 2010 for EFEU by Erich Frühstück. This code remains in the
	public domain. 
*/

#include <EFEU/MD5Context.h>

static void MD5Context_transform (uint32_t buf[4], const unsigned char in[64]);

/*
	Little-endian byte-swapping routines.  Note that these do not
	depend on the size of datatypes, nor do they require us to
	detect the endianness of the machine we are running on.  It is
	possible they should be macros for speed, but I would be
	surprised if they were a performance bottleneck for MD5. 
*/

static uint32_t getu32 (addr) const unsigned char *addr;
{
...
void MD5Context_update (MD5Context *ctx, unsigned char const *buf, size_t len)
{
	...
	if ( t ) {
		...
		memcpy(p, buf, t);
		MD5Context_transform (ctx->buf, ctx->in);
		buf += t;
		len -= t;
	}
...
void MD5Context_transform (uint32_t buf[4], const unsigned char inraw[64])
{
