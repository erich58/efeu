/*
64-Bit vorzeichenlose Ganzzahlobjekte mit variabler Satzlänge
...
{
	return io_ullread(io, data, 1);
}

static size_t write_data (const EfiType *type, const void *data, IO *io)
{
	return io_ullwrite(io, data, 1);
}

static int print_data (const EfiType *type, const void *data, IO *io)
{
	return io_xprintf(io, "%llu", *((T *) data));
}

EfiType Type_varsize = COMPLEX_TYPE(S, "uint64_t", sizeof(T), 0,
...
{
	*((char **) rval) = msprintf("%llu", *((T *) arg[0]));
}
...
static EfiFuncDef ctab[] = {
	...
	{ FUNC_RESTRICTED, &Type_varint, S " ()", c_int64 },
