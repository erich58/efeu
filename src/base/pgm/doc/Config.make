...
efeu_libvar iorl -ml RL_LIB md efmath efx efm -lm

foreach -A files -d . -S h 'mf_file $tg $src'
foreach -s c 'mf_cc -I. -c $src'

mf_var OBJ "efeudoc.o skip.o parse.o p_reg.o p_block.o copy.o docsym.o" \
	...
	"test.o" \
	"DocDrv.o" \
	"html.o html_open.o html_putc.o html_cmd.o html_env.o html_cpush.o" \
	"LaTeX.o latex.o latex_putc.o latex_cmd.o latex_env.o" \
	"mroff.o mroff_par.o mroff_var.o mroff_putc.o mroff_cmd.o mroff_env.o" \
	"term.o term_var.o term_putc.o term_cmd.o term_env.o" \
	"Doc.o docout.o" \
	"listarg.o f_section.o setup.o"

mf_var CFLAGS "-DCFGPATH='\".:\$(CFGDIR)\"'"
mf_cc -o '$(BIN)/efeudoc' -mo OBJ -md DEP -ml LIB
...
mf_var DOCBUF "DocBuf.o DocBuf_copy.o"

mf_var S2DOBJ "src2doc.o s2d_skip.o s2d_decl.o s2d_parse.o" \
	"s2d_data.o s2d_eval.o s2d_copy.o" \
	"s2d.o s2d_std.o s2d_src.o s2d_sh.o \$(DOCBUF)"

mf_var EMOBJ "efeuman.o \$(DOCBUF)"
mf_cc -o '$(BIN)/src2doc' -mo S2DOBJ -md DEP -ml LIB
mf_cc -o '$(BIN)/efeuman' -mo EMOBJ -md DEP -ml LIB
